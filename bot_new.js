  ...
  bot.action(/pubg:uc:(\d+):(\d+)/, async (ctx) => {
    try {
      ...
      if (userBalance < price) {
        ...
        await ctx.answerCbQuery(`❌ Yetarli mablag' mavjud emas! Sizning balansingiz: ${userBalance.toLocaleString()} so'm`);
        ...
        return sendOrUpdateMenu(
          ...
          `ℹ Eng arzon UC paketi: *${Math.min(...Object.values(UC_PRICES)).toLocaleString()} so'm*`,
          keyboard
        );
      }
      ...
      await ctx.answerCbQuery();
      await ctx.reply(`🎮 *PUBG UC Sotib Olish*\n\n` +
        `💎 UC Miqdori: *${ucAmount.toLocaleString()} UC*\n` +
        `💳 Narxi: *${price.toLocaleString()} so'm*\n\n` +
        `Iltimos, PUBG o'yinidagi ID raqamingizni yuboring:`, {
        parse_mode: 'Markdown',
        reply_markup: {
          inline_keyboard: [
            [{ text: '❌ Bekor qilish', callback_data: 'cancel_purchase' }]
          ]
        }
      });
    } catch (error) {
